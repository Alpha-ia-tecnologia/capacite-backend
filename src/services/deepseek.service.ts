...
export async function generateDevolutiva(
  ...
  const categoryDetails = ranking
    .map((cat, i) => {
      ...
      return `${i + 1}. ${cat.categoryName}: Score ${cat.score}/12 (Gap: ${cat.gap}) - ${level}`;
    })
    .join('\n');
  ...
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content: `Você é um consultor especializado em desenvolvimento de liderança do Global Leadership Summit.
...
Responda em português brasileiro.`,
    ...
    {
      role: 'user',
      content: `Gere uma devolutiva personalizada para a organização "${organizationName}" com os seguintes resultados:

${categoryDetails}

Prioridade 1 (maior necessidade): ${priority1.categoryName} (Gap: ${priority1.gap})
${priority2 ? `Prioridade 2 (alavanca): ${priority2.categoryName} (Gap: ${priority2.gap})` : 'Sem prioridade 2 identificada.'}

Para cada categoria prioritária, inclua:
...
export async function generateTrilhaSuggestions(
  priority1CategoryId: number,
  priority2CategoryId: number | null,
  organizationType: string
): Promise<{ trilha1: TrilhaSuggestion; trilha2: TrilhaSuggestion }> {
  const orgContext = getOrgContext(organizationType);
...
${orgContext}
Sugira trilhas de palestras do Summit baseadas nas categorias prioritárias.
REGRA ABSOLUTA: Use APENAS palestras e palestrantes do catálogo oficial fornecido. NUNCA invente, sugira ou mencione palestrantes ou palestras que não estejam na lista.
Responda APENAS com JSON válido, sem markdown.`,
    },
    {
      role: 'user',
      content: `Baseado nas prioridades identificadas:
- Prioridade 1: ${cat1?.name || 'Categoria ' + priority1CategoryId}
${cat2 ? `- Prioridade 2: ${cat2.name}` : ''}

Sugira duas trilhas de palestras no formato JSON:
{
  "trilha1": {
    "name": "nome da trilha",
...
Catálogo OFICIAL (IDs válidos: lp1-3, pcc1-3, ci1-3, ede1-3, mir1-3, rse1-3, pvl1-3):
${catalogStr}

Trilha 1 deve ser de IMPACTO (ação rápida, 3-4 palestras).
Trilha 2 deve ser de APROFUNDAMENTO (reflexão, 3-4 palestras).
Ambas devem focar nas categorias prioritárias mas podem incluir palestras complementares da lista fornecida.
NÃO use IDs como p1, p2, gls23. Use APENAS os IDs do catálogo acima (lp1, pcc2, ci3, etc).`,
    },
  ];

  const response = await chatCompletion(messages, 0.5);

  // Mapeamento de categoryId para prefixo de palestra
  ...
  function filterValidIds(ids: string[]): string[] {
    return ids.filter(id => PALESTRA_IDS_VALIDOS.includes(id));
  }

  try {
    const parsed = JSON.parse(response);
    // Validar que IDs retornados pela IA existem no catálogo
    if (parsed.trilha1) parsed.trilha1.palestraIds = filterValidIds(parsed.trilha1.palestraIds);
    if (parsed.trilha2) parsed.trilha2.palestraIds = filterValidIds(parsed.trilha2.palestraIds);
    return parsed;
  } catch {
    // Fallback: usar IDs do catálogo oficial baseados na categoria
    const fallbackIds1 = [`${prefix1}1`, `${prefix1}2`, `${prefix1}3`].filter(id => PALESTRA_IDS_VALIDOS.includes(id));
    const fallbackIds2 = [`${prefix2}1`, `${prefix2}2`, `${prefix2}3`].filter(id => PALESTRA_IDS_VALIDOS.includes(id));

    return {
      trilha1: {
        name: `Impacto Rápido - ${cat1?.name || 'Liderança'}`,
        description: 'Comece por aqui se você quer gerar mudanças visíveis em 2-3 semanas.',
        type: 'IMPACTO',
        palestraIds: fallbackIds1,
        reasoning: 'Trilha focada na categoria com maior necessidade identificada.',
      },
      trilha2: {
        name: `Aprofundamento - ${cat2?.name || cat1?.name || 'Liderança'}`,
        description: 'Comece por aqui se você quer consolidar aprendizados e criar raízes profundas.',
        type: 'APROFUNDAMENTO',
        palestraIds: fallbackIds2,
        reasoning: 'Trilha focada em reflexão e consolidação.',
      },
    };
  }
}
...
export async function generatePalestraQuestions(
...
Responda APENAS com JSON válido, sem markdown.`,
    ...
    {
      role: 'user',
      content: `Gere 5 perguntas de facilitação para processamento em grupo da palestra:
- Título: "${palestraTitle}"
- Palestrante: ${speaker}
- Categoria: ${categoryName}
${customContext ? `- Contexto específico da organização: ${customContext}` : ''}
